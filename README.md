...
| **AI 数据权限** | AI 助手入口受模块权限控制，普通用户查询主机 / K8s 数据时自动按服务器分组与 K8s 集群分组裁剪可见范围 |
...
AI 助手不是全局透视入口，平台对 AI 查询采用“模块权限 + 资源范围”双层控制：

- **模块权限**：用户必须具备 `agent.view` 才能看到并使用 AI 助手入口。
- **主机范围**：普通用户通过 AI 查询主机指标、主机巡检、SSH 目标主机时，只能访问管理员分配的 CMDB 服务器分组。
- **K8s 范围**：普通用户通过 AI 查询 Kubernetes 节点、Pod、Deployment、Service、命名空间时，只能看到管理员分配的 K8s 集群分组。
- **超管例外**：超级管理员默认不受上述资源范围限制。
- **管理入口**：管理员可在用户管理中分别给用户分配模块角色、服务器分组和 K8s 集群分组，避免普通账号通过 AI 越权查询全部资产。

这套权限同时作用于 AI 助手、K8s 页面接口和部分运维工具调用，保证“页面看不到的数据，AI 也不能查到”。
