...
*支持注册审批流、登录失败锁定、忘记密码，管理员首次启动自动创建*

### 仪表盘
![仪表盘](screenshots/02_dashboard.png)
*系统总览：Loki / Prometheus / AI 连接状态，24h 错误趋势，服务健康矩阵*
...
*SSH 读取远端慢日志，支持时间段筛选，drain3 SQL 模板聚合，AI 流式优化建议*

### CMDB 巡检
![CMDB 巡检](screenshots/05_cmdb.png)
*Prometheus 自动发现主机，实时 CPU/内存/磁盘/负载指标，阈值巡检一键导出 Excel*
...
### 用户管理
![用户管理](screenshots/08_admin.png)
*注册审批、按模块授权（view/operate）、解锁账号、操作审计日志*
...
| 模块 | 功能 |
|------|------|
| **仪表盘** | 系统总览、错误 Top10 服务、服务健康状态矩阵、连接状态实时检测 |
...
│  (指标+主机发现) │   │  Redis  (Session + 登录失败计数)      │
│  AI Provider     │   │  ./reports/  (日报 JSON)              │
│  (Claude/OpenAI) │   │  ./data/     (SQLite 数据库)          │
│  飞书 / 钉钉     │   │  cmdb_hosts.json (CMDB 扩展字段)      │
└──────────────────┘   └──────────────────────────────────────┘
```
...
4. 切换 **模板聚类** 标签，查看 Drain3 自动归纳的日志模板

### MySQL 慢日志分析

1. 进入「慢日志分析」页面
2. **配置凭证**（三种方式任选）：
   - **CMDB 自动**：主机在 CMDB 中已绑定凭证，直接选 IP 即可
   - **凭证库**：从凭证管理库中选择已保存的凭证
   - **手动输入**：直接填写用户名和密码
3. **添加分析目标**：在 IP 输入框输入主机 IP（支持 CMDB 自动补全），可批量添加多台
4. **设置时间段**：选择起始/结束日期，或使用快捷按钮（今天 / 昨天 / 近7天 / 近30天 / 不限）
5. 点击「**获取并解析**」，系统并行 SSH 读取所有目标主机的慢日志并解析
6. 结果按主机分 Tab 展示，切换三个视图：
   - **慢查询列表**：按耗时排序，可展开查看完整 SQL
   - **SQL 聚合**：drain3 自动识别 SQL 模板，统计次数 / 总耗时 / 最大耗时 / 扫描行数
   - **AI 分析建议**：流式输出根因分析和加索引/改写 SQL 等优化建议

> 凭证信息通过 sessionStorage 持久化，页面刷新后无需重新输入。

### 运维日报 + 定时推送

- **手动生成**：进入「分析报告」→ 点击「立即生成日报」，AI 流式输出健康评分和分析
- **手动推送**：报告生成后点击「飞书推送」或「钉钉推送」
- **定时自动推送**：配置 `SCHEDULE_CRON` 和 `SCHEDULE_CHANNELS` 后自动推送，无需手动操作

### 主机巡检

1. 进入「CMDB 巡检」→ 切换「巡检报告」标签
2. 点击「全量巡检」，系统从 Prometheus 拉取最新指标并执行阈值检查
3. 点击「AI 分析」，AI 逐台列出每个异常主机的 IP 和具体问题
4. 点击「下载 Excel」导出三页式报告（巡检概况 / 全部主机明细 / 异常项明细）

### SSH 终端

1. 进入「SSH 终端」页面
2. 点击主机旁的「SSH」按钮，或手动输入 IP 新建连接
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET  | `/api/slowlog/hosts` | CMDB 中有 SSH 凭证的主机列表 |
...
- 确认主机 IP、端口、用户名/密码正确
- 确认 SSH 账号有读取慢日志文件的权限：`sudo ls -la /mysqldata/mysql/data/3306/mysql-slow.log`
- 如果文件需要 root 权限，在 CMDB 凭证中使用 root 账号或配置 sudo 无密码
