...
> 本文件是 Codex CLI 读取的副本，内容与 Claude Code 侧的
> `.claude/skills/codegraph-usage/SKILL.md` 保持同步。以后改规则时两处一起改，
> 避免两个工具用的规则慢慢分叉。Codex 项目级 skill 目录约定是
> `<repo_root>/.agents/skills/<name>/SKILL.md`（对应 Claude 的 `.claude/skills/`）。
...
Codex 支持通过一行脚本安装（该 README 明确列出 Codex 是受支持平台之一）：
...
Claude Code 侧则是通过插件市场装（两边各装各的，互不影响）：
