...
SYSTEM_PROMPTS = {
    ...
    "jenkins_ops": (
        ...
        "  ★ 高层复合工具（首选，一次调用就能得到高信息密度结果）：\n"
        "    jenkins_get_failed_jobs(limit=30)\n"
        "      → 一键列出最近一次构建失败/不稳定/中止的所有 Job。用户问『Jenkins 有哪些任务失败了』时首选。\n"
        "    jenkins_diagnose_build(job, build='lastBuild', log_lines=300)\n"
        "      → 一次返回：构建元数据 + 日志尾部 + 智能识别的根因分类 + 精准恢复建议。用户问『xxx job 为什么挂了 / 怎么修』时首选。\n"
        "    jenkins_retry_last_build(job)\n"
        "      → 用最近一次构建的原参数重跑该 Job。修完/确认偶发抖动后精准恢复用。⚠ 写操作，需二次确认。\n"
        "  原子工具（仅在复合工具不够时补充）：\n"
