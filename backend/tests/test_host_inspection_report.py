...
class HostInspectionReportTests(unittest.IsolatedAsyncioTestCase):
    async def test_collect_inspect_data_uses_ssh_python_fallback_when_prometheus_missing(self):
        ...
        report_builder.load_hosts_list = lambda: [{"ip": "10.0.0.8", "hostname": "app-1", "group": "g1"}]
    ...
    async def test_inspect_excel_uses_runtime_status_columns(self):
        ...
        self.assertEqual(values[0], "警告")
        self.assertEqual(values[12], "/")
        self.assertEqual(values[13], 80)
        self.assertEqual(values[14], "80.0 / 100.0")
        self.assertIn("磁盘 /: 80%", values[21])
