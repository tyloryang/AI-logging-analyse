...
export const api = {
  // 服务列表
  getServices:        (params = {}, config = {}) => http.get('/services', { ...config, params }),
  getServicesGrouped: (params = {}, config = {}) => http.get('/services/grouped', { ...config, params }),
  getServiceErrorCounts: (hours = 24, config = {}) => http.get('/services/error-counts', { ...config, params: { hours } }),
  ...
  getMetricPanels: () => http.get('/observability/metric-panels'),
  saveMetricPanels: (data) => http.put('/observability/metric-panels', data),
  ...
  ansibleCreatePlaybook: (data)     => http.post('/ansible/playbooks', data),
  ansibleUpdatePlaybook: (id, data) => http.put(`/ansible/playbooks/${id}`, data),
  ansibleDeletePlaybook: (id)       => http.delete(`/ansible/playbooks/${id}`),
  ansibleRunPlaybook:    (id, data) => http.post(`/ansible/playbooks/${id}/run`, data),
  ansibleConfig:         ()         => http.get('/ansible/config'),
  ansibleSaveConfig:     (data)     => http.put('/ansible/config', data),
  ansibleCheckConfig:    ()         => http.post('/ansible/config/check'),
  // 事件墙
  ...
  kafkaOverview:       (id, refresh = false) => http.get(`/kafka/clusters/${id}/overview`, { params: refresh ? { refresh: true } : {} }),
  kafkaTopics:         (id, internal = false, refresh = false) => http.get(`/kafka/clusters/${id}/topics`, { params: { include_internal: internal, ...(refresh ? { refresh: true } : {}) } }),
  ...
  kafkaGroups:         (id, refresh = false) => http.get(`/kafka/clusters/${id}/groups`, { params: refresh ? { refresh: true } : {} }),
